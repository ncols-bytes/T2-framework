...
class HistogramHelper():
    ...
    def gen_histogram_from_entities(self, entities):
        histogram = np.zeros([len(entities), self.histogram_len + 1], dtype=float)
        for col_i, col in enumerate(entities):
            hist = []
            cell_list = [cell[1][1] for cell in col]
            cell_set = set(cell_list)
            if len(cell_set) > self.histogram_len:
                continue
            ...
            histogram[col_i][0] = len(hist) / self.histogram_len
    ...
    def reformat_mysql_histogram_buckets(self, one_histogram, histogram_num_sort):
        ...
        if mode == 'singleton':
            ...
            histogram.insert(0, len(histogram)/self.histogram_len)
            histogram = histogram + [0] * (self.histogram_len+1 - len(histogram))
        else:
            histogram = [0] * (self.histogram_len+1)
        return histogram
